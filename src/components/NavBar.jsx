  ...
  return (
    <div className="fixed w-full h-[80px] flex justify-between items-center px-4 bg-[#0a192f] text-gray-50 z-10">
      ...
      <ul
        className={
          ham
            ? "absolute top-0 left-0 w-screen h-screen bg-[#0a192f] flex flex-col justify-center items-center md:hidden"
            : "hidden"
        }
      >
        {NAVBAR_ITEMS.map((item, index) => (
          <Link
            ...
            onClick={handleHam}
          >
            <li className="py-6 text-4xl">{item}</li>
          </Link>
        ))}
      </ul>
      ...
      <ul className="hidden lg:fixed lg:top-[44%] lg:left-0 lg:flex lg:flex-col">
        {CONTACTS.map((contact, index) => (
          <a
            key={index}
            href={contact.link}
            className={`w-[160px] h-[60px] flex ml-[-100px] hover:ml-[-10px] duration-300 ease-in-out`}
